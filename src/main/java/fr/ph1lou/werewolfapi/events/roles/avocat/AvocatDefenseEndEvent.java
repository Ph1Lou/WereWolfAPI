package fr.ph1lou.werewolfapi.events.roles.avocat;

import fr.ph1lou.werewolfapi.annotations.statistics.StatisticsEvent;
import fr.ph1lou.werewolfapi.annotations.statistics.StatisticsExtraInt;
import fr.ph1lou.werewolfapi.annotations.statistics.StatisticsPlayer;
import fr.ph1lou.werewolfapi.annotations.statistics.StatisticsTarget;
import fr.ph1lou.werewolfapi.player.interfaces.IPlayerWW;
import org.bukkit.event.Event;
import org.bukkit.event.HandlerList;
import org.jetbrains.annotations.NotNull;

@StatisticsEvent(key = "werewolf.avocat_defense_end")
public class AvocatDefenseEndEvent extends Event {

    private static final HandlerList HANDLERS_LIST = new HandlerList();

    private final IPlayerWW playerWW;
    private final IPlayerWW targetWW;
    private final int honorAmount;

    public AvocatDefenseEndEvent(IPlayerWW playerWW, IPlayerWW targetWW, int honorAmount) {
        this.playerWW = playerWW;
        this.targetWW = targetWW;
        this.honorAmount = honorAmount;
    }

    @StatisticsPlayer
    public IPlayerWW getPlayerWW() {
        return playerWW;
    }

    @StatisticsTarget
    public IPlayerWW getTargetWW() {
        return targetWW;
    }

    @StatisticsExtraInt
    public int getHonorAmount() {
        return honorAmount;
    }

    public static HandlerList getHandlerList() {
        return HANDLERS_LIST;
    }

    @NotNull
    @Override
    public HandlerList getHandlers() {
        return HANDLERS_LIST;
    }
}
