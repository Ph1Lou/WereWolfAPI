 ...
 * invoked when the owning {@link fr.ph1lou.werewolfapi.role.interfaces.IRole}'s
 * player is alive ({@code getPlayerWW().isState(StatePlayer.ALIVE) == true}).
 *
 ...
 * naturally with {@code @EventHandler(ignoreCancelled = true)}. It also composes
 * with {@link RequireAbility}: place both annotations to require an enabled
 * ability <em>and</em> an alive player (the most common combined guard).
 */
@Retention(RetentionPolicy.RUNTIME)
@Target(ElementType.METHOD)
public @interface RequireAlive {
}
